...
if __name__ == "__main__":
    ...
    #--predictions
    continuous_predictions_at_all_horizons = pd.read_csv("../../models/equally_weighted_hj_ensemble/all_predictions.csv")
    ensemble_quantiles                     = pd.read_csv("../../models/equally_weighted_hj_ensemble/all_quantiles.csv")
    ensemble_binary_quantiles              = pd.read_csv("../../models/equally_weighted_hj_ensemble/all_binary_quantiles.csv")
    ...
    #--truth
    ecdc   = pd.read_csv("../../raw_data/observed_data/ecdc_data.csv")
    ...
    truth_at_horizon = pd.read_csv("../../truth_tables/truths_at_each_horizon.csv")
    ...
    canada_truths = truth_at_horizon.loc[truth_at_horizon.question_id==11039]
    ax.scatter(canada_truths["date"], canada_truths["truth"], color="black",s=20)
    ...
    #--QID 10981
    cdc = pd.read_csv("../../raw_data/observed_data/cdc_data.csv")
    ...
    states_truths = truth_at_horizon.loc[truth_at_horizon.question_id==10981]

    #--HARDCODE. There are 25 states on 6/23 and so we will consider 25 states on 6/24
    states_truths.loc[ states_truths.horizon==1, "truth" ] = 26
    
    ax.scatter(states_truths["date"], states_truths["truth"], color="black",s=20)
    ...
    ax.set_xticks( np.array( list(xticks[:-3]) + [xticks[-2]]) )
    ...
    #--add truth marks
    usa_cases = truth_at_horizon.loc[truth_at_horizon.question_id==10979]
    ax.scatter(usa_cases["date"], usa_cases["truth"], color="black",s=20)
    ...
    #--add truth marks
    europe_cases = truth_at_horizon.loc[truth_at_horizon.question_id==10978]
    ax.scatter(europe_cases["date"], europe_cases["truth"], color="black",s=20)
    ...
    #--add truth marks
    europe_cases = truth_at_horizon.loc[truth_at_horizon.question_id==10975]
    ax.scatter(europe_cases["date"], europe_cases["truth"], color="black",s=20)
    ...
    for horizon,Q in quants.groupby(["horizon"]):
        ...
        ax.plot(   [resolve_time + datetime.timedelta(days=horizon)]*2, [Q.loc[0.10,"value"], Q.loc[0.90,"value"]], lw=2,alpha=0.65  )
        ax.scatter( resolve_time + datetime.timedelta(days=horizon), Q.loc[0.50,"value"],s=8)
    ...
    binary_predictions_at_all_horizons = pd.read_csv("../../models/equally_weighted_hj_ensemble/binary_predictions.csv")
