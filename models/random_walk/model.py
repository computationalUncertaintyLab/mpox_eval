...
class model(object):
    ...
    def numpyro_model(self,data,forecast=0):
        ...
        ll    = numpyro.sample("ll", dist.GaussianRandomWalk(scale=sigma,num_steps=T) , obs = data )

        if forecast > 0:
            err = numpyro.sample("err", dist.GaussianRandomWalk(scale=sigma,num_steps = forecast))
            y_prediction = numpyro.deterministic("y_prediction", data[-1] + err) 
...
if __name__ == "__main__":
    ...
    for (loc,qid,hor,cut,res), subset in training_data.groupby(["location","question_id","horizon","cut_point","resolve_time"]):
        ...
        #--number of days between cut point and resolution
        horizon_days = datetime.strptime(res,"%Y-%m-%d %H:%M:%S") - datetime.strptime(str(data.iloc[-1]["cut_point"]),"%Y-%m-%d %H:%M:%S")
        horizon_days = horizon_days.days
